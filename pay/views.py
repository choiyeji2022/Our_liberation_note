...
class success(APIView):
    def get(self, request):
        ...
        try:
            ...
            # DB에 객체 저장
            subscribe = Subscribe.objects.create(group=group, price=totalAmount, is_subscribe=True, type=orderName)
            end_date = subscribe.calculate_end_date()
            subscribe.save()
            payment = Payment.objects.create(user=user, group=group, amount=totalAmount, supplied_amount=suppliedAmount,)
            payment.save()
            print(80, payment)
